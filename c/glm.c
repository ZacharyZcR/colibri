...
static int gpu_cache_get(Model*m,int layer,ESlot*e,ColiCudaTensor**pg,ColiCudaTensor**pu,ColiCudaTensor**pd){
    if(!g_gc_cap){
        ...
        g_gc_cap=(int)((gb*1e9)/(double)(trip>0?trip:1));
        if(g_gc_cap<64) g_gc_cap=64;   /* must hold one group so collection can't evict a pending slot */
        int max_slots=(m->c.n_layers+1)*m->c.n_experts;   /* never over-allocate past the model */
        if(g_gc_cap>max_slots) g_gc_cap=max_slots;
...
static void expert_host_restore(Model *m, int layer, ESlot *s){
    ...
    if(s->g.cuda && s->u.cuda && s->d.cuda){
        ...
        if(!posix_memalign((void**)&slab,4096,(size_t)wtot+8192)){
            ...
            if(ok){   /* only repoint the tensors once every download has succeeded */
                wo=0; fo=0;
                for(int k=0;k<3;k++){
                    qt[k]->qf = qt[k]->fmt? NULL : (float*)(slab+wo);
                    qt[k]->q8 = (int8_t*)(slab+wo); qt[k]->q4 = slab+wo;
                    qt[k]->s = qt[k]->fmt ? fslab+fo : NULL;
                    wo+=wb[k]; if(qt[k]->fmt) fo+=qt[k]->O;
                }
                s->slab=slab; s->slab_cap=(int64_t)wtot+8192; s->fslab=fslab; s->fslab_cap=ftot; return;
            }
            compat_aligned_free(slab); free(fslab);
        }
    }
#endif
    expert_load(m,layer,s->eid,s,1);   /* fallback: disk */
}
...
static void *expert_load_run(void *opaque){
    ...
    for(int q=0;q<j->nmiss;q++)
        expert_load(j->model,j->layer,j->uniq[j->base+j->missk[q]],&j->model->ws[q],1);
    j->seconds=now_s()-t0;
    return NULL;
}
...
static void moe(Model *m, Layer *l, int layer, float *x, int S, float *out){
    ...
    for(int base=0;base<nu;base+=64){
        ...
        ColiCudaTensor *group_gt[64],*group_ut[64],*group_dt[64]; int group_dev[64];
#endif
        for(int j=0;j<nb;j++){ int eid=uniq[base+j]; ESlot *e=use[j];
            ...
            if(group_enabled && g_cuda_enabled && !omp_in_parallel()){
                /* Run on GPU via the group path if resident, or (GPU_ONLY) via
                 * the on-demand VRAM cache -- batching cache experts too. */
                ColiCudaTensor *cg=NULL,*cu=NULL,*cd=NULL; int gdev=-1;
                if(e->g.cuda_eligible && e->u.cuda_eligible && e->d.cuda_eligible){
                    cg=e->g.cuda; cu=e->u.cuda; cd=e->d.cuda; gdev=e->g.cuda_device;
                } else if(g_gpu_only && gpu_cache_get(m,layer,e,&cg,&cu,&cd)){
                    gdev=coli_cuda_tensor_device(cg);
                }
                if(gdev>=0){
                    group_e[ngroup]=e; group_gt[ngroup]=cg; group_ut[ngroup]=cu; group_dt[ngroup]=cd;
                    group_dev[ngroup]=gdev; group_n[ngroup]=nr;
...
#ifdef COLI_CUDA
        /* GPU expert group dispatch: compute the collected resident/cached experts
         * on the device(s), one grouped call per GPU, then scatter-add the results. */
        ...
        for(int di=0;di<g_cuda_ndev;di++) for(int q=0;q<ngroup;q++)
            if(group_dev[q]==g_cuda_devices[di]) dev_total[di]+=group_n[q];
        ...
        for(int di=0;di<g_cuda_ndev;di++){
            ...
            for(int q=0;q<ngroup;q++) if(group_dev[q]==device){
                int nc=dev_nc[di]++;
                dev_g[di][nc]=group_gt[q]; dev_u[di][nc]=group_ut[q]; dev_d[di][nc]=group_dt[q];
        ...
        for(int di=0;di<g_cuda_ndev;di++){
            ...
            for(int q=0;q<dev_nc[di];q++){
                ...
                if(!dev_ok[di]){
                    ...
                    if(!coli_cuda_expert_mlp(group_gt[gi],group_ut[gi],group_dt[gi],hh,xg,nr)){
                        expert_ffn(m,layer,e,xg,hh,gg,uu,nr);
                    }
                }
...
int main(int argc, char **argv){
    ...
    { double ram_env = getenv("RAM_GB")?atof(getenv("RAM_GB")):0.0;
      ...
      if(!getenv("PIN") && autopin && hist>=5000){
           ...
           * budget, so a large budget leaves most of VRAM unused. Grow the
           * pinned set to cover it (VRAM-resident experts release their host
           * backing, so this fills GPU memory without extra RAM pressure).
           * For CUDA_EXPERT_GB=auto, estimate the budget from free VRAM now. */
          if(g_cuda_enabled){
              double vgb = g_cuda_expert_gb;
              if(g_cuda_expert_auto){
                  size_t vfree=0; for(int i=0;i<g_cuda_ndev;i++){ size_t f=0,t=0;
                      if(coli_cuda_mem_info(g_cuda_devices[i],&f,&t)) vfree+=f; }
                  vgb = (double)vfree/1e9 - 2.0*g_cuda_ndev;   /* ~2 GB/device slack, matches pin_load */
                  if(vgb<0) vgb=0;
              }
              if(vgb > pin_gb) pin_gb = vgb;
          }
#endif
